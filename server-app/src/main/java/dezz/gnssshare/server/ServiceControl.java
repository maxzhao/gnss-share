...
final class ServiceControl {
    static final String EXTRA_START_FROM_TILE =
            "dezz.gnssshare.server.extra.START_FROM_TILE";

    private static final String TAG = "ServerServiceControl";
    private static final String SERVICE_STATE_FILE = "gnss_server_service_state";
    ...
    static void publishServiceRunning(Context context) {
        ...
        try {
            ...
            Log.e(TAG, "Unable to publish the GNSS server service state", exception);
        }
    ...
    static boolean startService(Context context) {
        try {
            ContextCompat.startForegroundService(
                    context,
                    new Intent(context, GNSSServerService.class)
            );
            return true;
        } catch (RuntimeException exception) {
            Log.e(TAG, "Unable to start the GNSS server service", exception);
            return false;
        }
    }
    ...
    static void requestTileRefresh(Context context) {
        TileService.requestListeningState(
                context,
                new ComponentName(context, GNSSServerTileService.class)
        );
    }
